...
class PDF extends FPDF
{
    ...
    function CreateTable($header, $data)
    {
        ...
        $this->SetFont('Arial', 'B', 12);
        foreach ($header as $i => $col) { $this->Cell($widths[$i], 10, $col, 1, 0, 'C'); }
        $this->Ln();
        $this->SetFont('Arial', '', 12);
        foreach ($data as $row) {
            foreach ($row as $i => $col) { $this->Cell($widths[$i], 10, $col, 1, 0, 'C'); }
            $this->Ln();
        }
    }
...
// ✅ IDOR Protection: Patient can only view their own reports
$patientUid = (int)($_SESSION['uid'] ?? 0);
$sql = mysqli_query($connect, "SELECT doctors.doctorName as docname, appointment.* FROM appointment JOIN doctors ON doctors.id=appointment.doctorId WHERE apid = $id AND appointment.userId = $patientUid");
if (mysqli_num_rows($sql) > 0) {
    $row = mysqli_fetch_assoc($sql);
    $data = array(
        array($row['apid'], $row['patient_Name'], $row['appointmentDate'], $row['docname'], $row['doctorSpecialization']),
    );
} else { die("Access denied: You can only view your own reports."); }

// ✅ FIX: البحث بـ apid فقط
...
if (mysqli_num_rows($sql2) > 0) {
    ...
    $Scan      = $row2['Scan']         ?? 'The Scan Has Not Been Written Yet';
} else {
